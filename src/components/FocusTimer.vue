<template>
  ...
  Trees grown: {{ Math.floor((selectedDuration - timeLeft) / 60) }}  <!-- Change from 3600 to 60 -->
</div>
              </div>
            </div>
          </div>
...
/>
  ...
  <span>1 min</span>     <!-- Change from "30 min" -->
  <span>2 min</span>     <!-- Change from "1 hour" -->
  <span>3 min</span>     <!-- Change from "2 hours" -->
  <span>4 min</span>     <!-- Change from "3 hours" -->
  <span>5 min</span>     <!-- Change from "4 hours" -->
</div>
            </div>
...
<div v-if="isRunning" class="relative mb-8">
  ...
  <div class="relative z-10 p-8">
    <h3 class="text-2xl font-bold text-stone-900 mb-6 text-center">Focus Session</h3>

    
    <!-- Task List (Interactive during focus) -->
    <div v-if="folderTasks.length > 0">
      <label class="block text-sm font-medium text-stone-700 mb-4">Tasks you can work on</label>
      <div class="space-y-3 max-h-80 overflow-y-auto custom-scrollbar mb-8">
        ...
        >
          ...
          <div class="flex-1">
            ...
            <div v-if="task.dueDate" class="text-xs text-stone-600 mt-1">
              Due: {{ formatTaskDueDate(task.dueDate) }}
            </div>
          </div>
          
          <!-- Completion Badge -->
          <div v-if="task.completed" class="text-lg">
            ✓
          </div>
        </div>
      </div>
...
<div v-if="!isRunning" class="relative mb-8">
  ...
  <div class="relative z-10 p-8">
    <h3 class="text-2xl font-bold text-stone-900 mb-6">Select Focus Topic</h3>
    
    <!-- Folder Selection -->
    <div class="mb-6">
      <label class="block text-sm font-medium text-stone-700 mb-3">What are you working on?</label>
      <div class="relative">
        <select
        ...
        >
          <option value="" class="bg-white text-stone-900">Select what you're working on</option>
          <option 
    ...
    <div v-else-if="!selectedFolderId" class="text-center py-8 text-stone-600">
      ...
      <p class="font-medium">Select what you're working on</p>
    </div>
  </div>
</div>
      </div>
...
const todaySessions = computed(() => {
  ...
  return props.focusSessions ? props.focusSessions.filter(session => 
    new Date(session.completedAt).toDateString() === today
  ) : []
})

const todayTotalTime = computed(() => {
  return todaySessions.value.reduce((total, session) => total + (session.duration || 0), 0)
})
...
const stopTimer = () => {
  ...
  if (elapsedTime >= 60) { // 1 minute = 60 seconds
  ...
  console.log('Emitting focus time update:', elapsedTime, 'for folder:', selectedFolderId.value)
...
const toggleTaskCompletion = (task) => {
  ...
  if (taskIndex !== -1) {
    folderTasks.value[taskIndex] = {
      ...
      completedAt: !folderTasks.value[taskIndex].completed ? new Date().toISOString() : null
    }
  }
...
const finishSession = () => {
  ...
  console.log('Emitting focus time update:', selectedDuration.value, 'for folder:', selectedFolderId.value)
...
const growTree = () => {
  emit('tree-grown', {
    type: 'oak',
    duration: 60, // Change from 3600 to 60 (1 minute)
    source: 'timer',
